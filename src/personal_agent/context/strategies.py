...
def _avoid_splitting_tool_group(messages: list[Message], split: int) -> int:
    """Move a split index so it does not fall inside a tool-call group.
    ...
    message, keeping ``messages[split:]`` orphans the parent assistant message
    and causes provider API errors.

    The previous implementation walked the split *left* to the parent assistant
    so the whole group was kept. When the group's parent sat near the front of
    ``messages`` (e.g. a long tool run early in the conversation), that dragged
    the entire conversation back into the window, silently violating
    ``max_messages``. Instead advance the split *past* the trailing tool result
    so the group is dropped whole and the cap still holds. This also covers the
    already-orphaned leading-run case: a ``tool`` message at index 0 is skipped.
    """
